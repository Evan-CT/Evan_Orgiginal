...
@implementation CTCustomSegmentView
...
- (void)customData {
    ...
    if (_titlesCustomColor == nil) {
        _titlesCustomColor = [[UIColor blackColor] colorWithAlphaComponent:0.5];
    }
    
    if (_titlesHeightLightColor == nil) {
        _titlesHeightLightColor = [UIColor blackColor];
    }
    
    if (_backgroundHeightLightColor == nil) {
        _backgroundHeightLightColor = [UIColor whiteColor];
    }
...
- (void) createBottomLabels {
    for (int i = 0; i < _titles.count; i ++) {
        UILabel *tempLabel = [self createLabelWithTitlesIndex:i textColor:_titlesCustomColor];
...
- (void) createTopLables {
    ...
    _heightColorView.backgroundColor = _backgroundHeightLightColor;
    _heightColorView.layer.cornerRadius = 5;
    _heightColorView.layer.borderColor = [UIColor grayColor].CGColor;
    _heightColorView.layer.borderWidth = 1;
...
- (void) createTopControls
{
    for (int i = 0; i < _titles.count; i ++) {
        ...
        UIControl *tempControl = [[UIControl alloc] initWithFrame:tempFrame];
        tempControl.tag = i;
        [tempControl addTarget:self action:@selector(tapButton:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)tapButton:(UIButton *) sender {
    ...
    [UIView animateWithDuration:_duration animations:^{
        ...
        if (_isAnimation) {
            [weak_self shakeAnimationForView:_heightColorView];
        }
    }];
}
